...
//#define BYTE_ADDRESS ((Address.Page + (Address.Block + (Address.Zone * NAND_ZONE_SIZE)) * NAND_BLOCK_SIZE) * (NAND_PAGE_SIZE + NAND_SPARE_AREA_SIZE) + Address.OffsetInPage)
#define COLUMN_ADDRESS (Address.OffsetInPage)
/**
  * @}
  */ 
...
/*!< INT2 NAND pin configuration */  
  ...
  FSMC_NANDInitStructure.FSMC_TCLRSetupTime = 0x02; // 15ns
  FSMC_NANDInitStructure.FSMC_TARSetupTime = 0x02; // 10ns
...
*/
		//i1=COL_ADDR_1st_CYCLE(COLUMN_ADDRESS);
		//i2=COL_ADDR_2nd_CYCLE(COLUMN_ADDRESS);
		//i3=ROW_ADDR_3rd_CYCLE(ROW_ADDRESS);
		//i4=ROW_ADDR_4th_CYCLE(ROW_ADDRESS);
		*(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) = COL_ADDR_1st_CYCLE(COLUMN_ADDRESS);
    *(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) = COL_ADDR_2nd_CYCLE(COLUMN_ADDRESS);
  ...
  while((NumPageToRead != 0x0) && (addressstatus == NAND_VALID_ADDRESS))
  {
    ...
    *(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) = ROW_ADDR_4th_CYCLE(ROW_ADDRESS);
		...
		*(__IO uint8_t *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_READ_TRUE;
		...
		//delay(5); // Wait for finish
		//while(NAND_ReadStatus() != NAND_READY) {};
...
uint32_t NAND_EraseBlock(NAND_ADDRESS Address)
{
  *(__IO uint8_t *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_ERASE0;
  ...
  //*(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) = ADDR_1st_CYCLE(ROW_ADDRESS);
	// Only Row address is needed. 1st is page address
  *(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) = ROW_ADDR_3rd_CYCLE(ROW_ADDRESS);
  *(__IO uint8_t *)(Bank_NAND_ADDR | ADDR_AREA) = ROW_ADDR_4th_CYCLE(ROW_ADDRESS);

  *(__IO uint8_t *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_ERASE1; 
		delay(3); // Wait for finish
...
void NAND_ConvertOffsetToAddress(vu32 offset, vu32 page_size, NAND_ADDRESS* addr) {
	...
	addr->Zone = offset / zone_bytes;
	addr->Block = (offset % zone_bytes) / block_bytes;
